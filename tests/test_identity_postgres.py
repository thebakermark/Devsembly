...
async def test_concurrent_owner_removal_preserves_one_active_owner(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    assert TEST_DATABASE_URL is not None
    isolated_engine = create_async_engine(TEST_DATABASE_URL, pool_pre_ping=True)
    isolated_sessions = async_sessionmaker(
        isolated_engine,
        expire_on_commit=False,
        class_=AsyncSession,
    )
    monkeypatch.setattr("devsembly.identity_api.SessionFactory", isolated_sessions)
    try:
        async with isolated_sessions() as session, session.begin():
            await session.execute(
                text(
                    "TRUNCATE outbox_events, audit_events, workflow_step_attempts, "
                    "workflow_steps, workflow_runs, decisions, cost_evaluations, "
